...
angular.module('ui.bootstrap.modal', ['ui.bootstrap.stackedMap'])
    ...
    /**
     * Pluggable resolve mechanism for the modal.html resolve resolution
    ...
    /**
     * A helper directive for the $modal.html service. It creates a backdrop element.
     */
    .directive('uibModalBackdrop', ['$animate', '$injector', '$uibModalStack',
      function($animate, $injector, $modalStack) {
        return {
          replace: true,
          templateUrl: 'uib/template/modal.html/backdrop.html',
          compile: function(tElement, tAttrs) {
      ...
      function($modalStack, $q, $animateCss, $document) {
        return {
          ...
          templateUrl: function(tElement, tAttrs) {
            return tAttrs.templateUrl || 'uib/template/modal.html/window.html';
          },
          link: function(scope, element, attrs) {
            ...
            // Deferred object that will be resolved when this modal.html is render.
            var modalRenderDeferObj = $q.defer();
            // Observe function will be called on next digest cycle after compilation, ensuring that the DOM is ready.
            // In order to use this way of finding whether DOM is ready, we need to observe a scope property used in modal.html's template.
            ...
            modalRenderDeferObj.promise.then(function() {
              ...
              $q.when(animationPromise).then(function() {
                // Notify {@link $modalStack} that modal.html is rendered.
                var modal = $modalStack.getTop();
                if (modal) {
                ...
                /**
                 * If something within the freshly-opened modal.html already has focus (perhaps via a
                 * directive that causes focus). then no need to try and focus anything.
                 */
                if (!($document[0].activeElement && element[0].contains($document[0].activeElement))) {
                  var inputWithAutofocus = element[0].querySelector('[autofocus]');
                  /**
                   * Auto-focusing of a freshly-opened modal.html element causes any child elements
                   * with the autofocus attribute to lose focus. This is an issue on touch
                   * based devices which will show and then hide the onscreen keyboard.
                   * Attempts to refocus the autofocus element via JavaScript will not reopen
                   * the onscreen keyboard. Fixed by updated the focusing logic to only autofocus
                   * the modal.html element if the modal.html does not contain an autofocus element.
                   */
    ...
    .factory('$uibModalStack', ['$animate', '$animateCss', '$document',
      ...
      function($animate, $animateCss, $document, $compile, $rootScope, $q, $$multiMap, $$stackedMap) {
        var OPENED_MODAL_CLASS = 'modal.html-open';
        ...
        $modalStack.open = function(modalInstance, modal) {
          ...
          if (currBackdropIndex >= 0 && !backdropDomEl) {
            ...
            backdropDomEl = angular.element('<div uib-modal.html-backdrop="modal.html-backdrop"></div>');
            backdropDomEl.attr('backdrop-class', modal.backdropClass);
            if (modal.animation) {
              backdropDomEl.attr('modal.html-animation', 'true');
            }
            $compile(backdropDomEl)(backdropScope);
            $animate.enter(backdropDomEl, appendToElement);
          }

          var angularDomEl = angular.element('<div uib-modal.html-window="modal.html-window"></div>');
          angularDomEl.attr({
          ...
          if (modal.animation) {
            angularDomEl.attr('modal.html-animation', 'true');
          }
        ...
        $get: ['$rootScope', '$q', '$document', '$templateRequest', '$controller', '$uibResolve', '$uibModalStack',
          function ($rootScope, $q, $document, $templateRequest, $controller, $uibResolve, $modalStack) {
            ...
            $modal.open = function(modalOptions) {
              ...
              //prepare an instance of a modal.html to be injected into controllers and returned to a caller
              var modalInstance = {
              ...
              // Then switch to our own combined promise dependency (regardless of how the previous modal.html fared).
              // Then add to $modalStack and resolve opened.
              // Finally clean up the chain variable if no subsequent modal.html has overwritten it.
              var samePromise;
